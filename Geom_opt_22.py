...
class Angle:
    ...
    def __repr__(self):
        return f"{self.atom_1} --- {self.atom_2} --- {self.atom_3} angle value in radians = {self.angle_value} angle value in degrees = {self.angle_value * 180 / 3.141592653589793} angle energy: {self.angle_energy}."
...
def compute_bond_energies(atoms,bond):
    ...
    if atoms[bond.atom_1-1].element == "C" and atoms[bond.atom_2-1].element == "C": #CALCULATING THE BOND ENERGIES FOR C-C BONDS 
        kb=300
        r0=1.53
...
def get_angles(atoms,bonds):
    ...
    angles.sort(key=lambda angle: (angle.atom_2, angle.atom_1, angle.atom_3))
...
def compute_G_mat_inverse(G_mat):

    eigenvalues, eigenvectors = np.linalg.eigh(G_mat)

    # SET A FILTER TO ELIMINATE 0 EIGEN VALUES
    non_zero_indices = np.abs(eigenvalues) > 1e-10
    non_zero_eigenvalues = eigenvalues[non_zero_indices]
    non_zero_eigenvectors = eigenvectors[:, non_zero_indices]

    # Or using matrix formulation
    Lambda_inv = np.diag(1 / non_zero_eigenvalues)
    G_mat_inv = non_zero_eigenvectors @ Lambda_inv @ non_zero_eigenvectors.T
...
def compute_GRMS(cartesian_gradient):
    
    num_atoms = cartesian_gradient.shape[0]

    GRMS = np.sqrt(np.sum(cartesian_gradient**2) / (3 * num_atoms))
...
def main():
    ...
    G_mat_inv=compute_G_mat_inverse(G_mat)
    ...
    E0=0
    j=0
    GRMS=compute_GRMS(cartesian_gradient)
    #while 10^(-3)<=GRMS:
    ...
    #while j<=2:
        ...
        alpha=1 #WE SET ALPHA TO BE ONE
        ...
        #ACTUALIZE THE ATOMIC COORDINATES 
        for i in range(len(atoms)):
        ...
        ##################################################################
        #print(np.shape(new_internal_coor),np.shape(internal_coor) )

        s_k=internal_coor
        #s_k=new_internal_coor-internal_coor
#
        for i in range(np.size(s_k)):
                
                s_k[i]=new_internal_coor[i]-internal_coor[i]
                if i>=len(bonds)+len(angles):
        #            #time.sleep(1)
        #            #print(i)
        #            #print("desired_internals[i]  :",desired_internals[i])
        #            #print("internals[i]  :",internal_1[i])
        #            #print("sqk[i]  :",s_q_k[i,0])
                    if abs(s_k[i])>np.pi and s_k[i]>0:
                        s_k[i]=s_k[i]-2*np.pi
                        #print("sqk[i]  :",s_q_k[i,0])
                    elif abs(s_k[i])>np.pi and s_k[i]<0:
                        s_k[i]=s_k[i]+2*np.pi
                        #print("sqk[i]  :",s_q_k[i,0])
        ...
        #COMPUTE THE ROOT MEAN SQUARE OF THE CARTESIAN GRADIENT
        GRMS=compute_GRMS(new_cartesian_gradient)

        with open(file_name+".out", "a") as file:
            file.write(f"\n ITERATION {j} GRMS= {GRMS} \n")
            file.write("\n")
        ...
        dot_s_y = np.dot(s_q_k.T, y_q_k)  
        
        dot_y_v = np.dot(y_q_k.T, v_q_k)  
       
        outer_s_s = np.outer(s_q_k, s_q_k)  
        outer_v_s = np.outer(v_q_k, s_q_k)  
        outer_s_v = np.outer(s_q_k, v_q_k)  
        ...
        #INITIALIZE THE INTERNAL COORDINATES FOR THE NEW ITERATION
        internal_coor=new_internal_coor

        Compute_tot_energy(atoms,bonds,angles,torsions,VDW_interactions,file_name)
        
        #INITIALIZE THE INTERNAL GRADIENT FOR THE NEW ITERATION
        internal_gradient=new_internal_gradient
        #print(f"iteration {j}")

    #FINALLY WRITE THE GEOMETRY AND THE CORRESPONDING ENERGIES
    with open(file_name+".out", "a") as file:
            file.write(f"\n ////////////////////////\n")
            file.write(" // OPTIMIZED GEOMETRY //\n")
            file.write(" ////////////////////////\n")
            file.write("\n")
            file.write(f"\n FINAL ATOMIC COORDINATES ({num_atoms}) AFTER {j} ITERATIONS. \n")
            file.write("\n")
            for atom in atoms:
                file.write(f"Atom {atom.atm_num}: {atom.element} {atom.x_coor:.6f}, {atom.y_coor:.6f}, {atom.z_coor:.6f} \n") 
            file.write("\n")
